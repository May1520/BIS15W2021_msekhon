...
## `mutate()`  
Mutate allows us to create a new column from existing columns in a data frame (This allows for mathamatical computation in the columns). We are doing a small introduction here and will add some additional functions later. Let's convert the length variable from cm to millimeters and create a new variable called length_mm.  
```{r}
fish %>% 
  mutate(length_mm = length*10) %>% 
  select(fish_id, length, length_mm)
```
[This created the new column length_mm which is length times 10.]
