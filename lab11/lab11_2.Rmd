...
legend position can be 'none' or "bottom" or "top" or"right", fill in labs controls the title of legends
```{r}
p+theme_linedraw()+
...
## Faceting: `facet_grid()`
As you can imagine, there are lots of options for faceting. Another useful facet type is `facet_grid`.  Use this for two categorical variables.
```{r}
p2+facet_grid(thermoregulation~class)
```

##Printing
used to export plots for use outside of R. 
```{r}
pdf("homerange")
p2
dev.off()
